...
export function ActionHomeBlue({ user, recentReferrals, whatsappUrl, monthStats }: ActionHomeBlueProps) {
    ...
    const referralLink = `https://ambassador.heguru.in/join/${user.referralCode}`
    ...
    return (
        <PageAnimate className="space-y-6 md:space-y-8 pb-10 font-[family-name:var(--font-outfit)]">
            {/* Option 1: Blue + Teal - Trusted Innovation */}
            <PageItem
                className="bg-gradient-to-br from-blue-700 via-blue-600 to-blue-800 rounded-[32px] md:rounded-[40px] p-6 md:p-10 text-white relative overflow-hidden shadow-[0_30px_60px_-15px_rgba(29,78,216,0.3)] min-h-[480px] md:min-h-[420px] flex flex-col justify-between"
            >
                ...
                <div className="relative z-10 w-full">
                    <div className="mb-8">
                        <motion.p
                            ...
                            className="text-blue-100 font-medium tracking-widest text-xs md:text-sm uppercase mb-1"
                        >
                            {greeting}
                        </motion.p>
                        <motion.h1
                        ...
                        <p className="text-blue-100/90 text-sm md:text-base mt-2 font-medium max-w-md">
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-6">
                        {/* Referrals Card - Cohesive Blue Theme */}
                        <div className="bg-gradient-to-br from-blue-500 to-blue-600 backdrop-blur-md border border-white/20 h-full p-4 rounded-[24px] text-white shadow-lg">
                            <div className="flex items-center gap-2 mb-3">
                                <div className="w-8 h-8 bg-white/20 rounded-lg flex items-center justify-center">
                                    <TrendingUp size={16} />
                                </div>
                                <span className="text-[10px] font-bold uppercase tracking-wider opacity-80">Referrals</span>
                            </div>
                            <div className="text-3xl font-extrabold mb-1">{displayCount}</div>
                            <div className="text-[10px] font-semibold opacity-70">
                                {displayCount >= 5 ? '5-Star Reached!' : `${5 - displayCount} more to 5-Star`}
                            </div>
                        </div>

                        {/* Fee Benefit Card - Semantic Emerald for Money */}
                        <div className="bg-gradient-to-br from-emerald-400 to-emerald-500 text-white border-white/20 h-full p-4 rounded-[24px] shadow-lg">
                            <div className="flex items-center gap-2 mb-3">
                                <div className="w-8 h-8 bg-white/20 rounded-lg flex items-center justify-center">
                                    <Wallet size={16} />
                                </div>
                                <span className="text-[10px] font-bold uppercase tracking-wider opacity-80">
                                    {user.role === 'Alumni' ? 'Benefit' : 'Fee Benefit'}
                                </span>
                            </div>
                            <div className="text-3xl font-extrabold mb-1">{user.yearFeeBenefitPercent}%</div>
                            <div className="text-[10px] font-semibold opacity-70">Applied to Fee</div>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-3 mt-4">
                    <motion.a
                        ...
                        whileHover="hover"
                        whileTap="tap"
                        className="flex-1 group relative flex items-center justify-center gap-3 bg-white text-blue-700 h-14 rounded-[20px] font-black text-[10px] uppercase tracking-[0.2em] shadow-xl hover:shadow-2xl transition-all overflow-hidden border border-white"
                    >
                ...
                <div className="text-center mt-3">
                    <p className="text-[10px] text-white/50 font-bold uppercase tracking-[0.3em]">Trusted Innovation Theme</p>
                </div>
            </PageItem>

            <PageItem className="grid grid-cols-2 gap-4">
                <Link href="/refer" className="block h-full">
                    <div className="group bg-gradient-to-br from-blue-600 to-blue-700 text-white rounded-[24px] p-6 flex flex-col items-center justify-center gap-3 shadow-[0_15px_30px_-10px_rgba(29,78,216,0.2)] transition-all relative overflow-hidden h-full min-h-[140px]">
                ...
                <Link href="/design/blue-teal/analytics" className="block h-full">
                    <div className="group bg-gradient-to-br from-teal-500 to-teal-600 text-white rounded-[24px] p-6 flex flex-col items-center justify-center gap-3 shadow-[0_15px_30px_-10px_rgba(20,184,166,0.2)] transition-all relative overflow-hidden h-full min-h-[140px]">
            ...
            <PageItem className="glass-panel rounded-[32px] md:rounded-[40px] border border-slate-200/50 dark:border-white/10 overflow-hidden">
                <div className="flex items-center justify-between p-6 border-b border-slate-100/50 dark:border-white/5">
                    ...
                    <div className="flex gap-2">
                        <Link href="/referrals" className="text-xs font-bold text-blue-600 uppercase tracking-wider flex items-center gap-1 hover:bg-blue-50 px-3 py-1.5 rounded-full transition-colors">
                ...
                {recentReferrals.length === 0 ? (
                    <div className="p-12 text-center">
                        ...
                        <Link
                            href="/refer"
                            className="inline-flex items-center gap-2 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white px-6 py-3 rounded-full font-bold text-sm transition-all shadow-lg hover:shadow-xl"
                        >
                            <UserPlus size={18} />
                            Make First Referral
                        </Link>
