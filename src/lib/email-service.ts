...
export const EmailService = {
    async sendWelcomeEmail(to: string, name: string, role: string) {
        ...
        try {
            const { data, error } = await resend.emails.send({
                from: process.env.EMAIL_FROM || 'Heguru Partners <onboarding@5starambassador.com>',
                to: [to],
                subject: 'Welcome to Heguru Ambassador Program! 🌟',
                html: `
                    <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
                        ...
                        <br/>
                        <a href="https://ambassador.heguru.in" style="background: #D97706; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px;">Go to Dashboard</a>
                    </div>
                `
            });
    ...
    async sendLeadAssignedEmail(to: string, leadName: string, campus: string) {
        ...
        try {
            await resend.emails.send({
                from: process.env.EMAIL_FROM ? `Heguru Partners <${process.env.EMAIL_FROM.split('<')[1] || process.env.EMAIL_FROM}` : 'Heguru Partners <leads@5starambassador.com>',
                to: [to],
    ...
    async sendPaymentConfirmation(to: string, name: string, amount: number, transactionId: string) {
        ...
        try {
            await resend.emails.send({
                from: process.env.EMAIL_FROM ? `Heguru Partners <${process.env.EMAIL_FROM.split('<')[1] || process.env.EMAIL_FROM}` : 'Heguru Partners <accounts@5starambassador.com>',
                to: [to],
    ...
    async sendSupportNewMessage(to: string, name: string, ticketSubject: string, messageSnippet: string, isFromAdmin: boolean) {
        ...
        try {
            ...
            await resend.emails.send({
                from: process.env.EMAIL_FROM ? `Heguru Partners <${process.env.EMAIL_FROM.split('<')[1] || process.env.EMAIL_FROM}` : 'Heguru Partners <support@5starambassador.com>',
                to: [to],
                subject: subject,
                html: `
                    <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
    ...
    async sendReportEmail(to: string, subject: string, htmlBody: string, reportTitle: string) {
        ...
        try {
            const from = process.env.EMAIL_FROM || 'Heguru Partners <reports@5starambassador.com>';
            const { data, error } = await resend.emails.send({
                from: from,
                to: [to],
                subject: subject,
                    ...
                    <div style="font-family: sans-serif; max-width: 800px; margin: 0 auto;">
                        ...
                        <div style="padding: 20px; border: 1px solid #E5E7EB; border-top: none; border-radius: 0 0 8px 8px;">
                            ...
                            <p style="margin-top: 30px; font-size: 12px; color: #6B7280; text-align: center;">
                                Generated automatically by Heguru Portal.
                            </p>
                        </div>
                    </div>
                `
    ...
    async sendReengagementEmail(to: string, name: string, badgeTier: string) {
        ...
        try {
            const { data, error } = await resend.emails.send({
                from: process.env.EMAIL_FROM || 'Heguru Partners <community@5starambassador.com>',
                to: [to],
                subject: 'We miss you at Heguru! 🚀',
                html: `
                    <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
                        ...
                        <p>Join our Heguru journey and help more students experience the 5-Star Education while earning exclusive benefits.</p>
                        <br/>
                        <a href="https://ambassador.heguru.in" style="background: #D97706; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px; font-weight: bold;">Return to Dashboard</a>
                    </div>
                `
            });
    ...
    async sendCampaignEmail(to: string, subject: string, htmlBody: string) {
        ...
        try {
            const { data, error } = await resend.emails.send({
                from: process.env.EMAIL_FROM || 'Heguru Partners <marketing@5starambassador.com>',
                to: [to],
                subject: subject,
                html: htmlBody
            });
    ...
    async sendEmailWithAttachment(to: string, subject: string, htmlBody: string, attachment: { filename: string, content: string | Buffer }, cc?: string[]) {
        if (!process.env.RESEND_API_KEY) {
            console.log(`[DEV MODE] Email Service: Sending Email with Attachment to ${to} (CC: ${cc?.join(', ') || 'none'})`);
            return { success: true, id: 'mock-id' };
        }
