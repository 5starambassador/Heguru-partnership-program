...
<body>
    <div class="container">
        <div class="header">
            <h1>HEGURU</h1>
            <p>5-Star Ambassador Program • Japan Design Language</p>
        </div>
...
export async function sendWelcomeEmail(to: string, name: string, referralCode: string) {
    try {
        await resend.emails.send({
            from: 'Heguru Ambassador <noreply@ambassador.heguru.in>',
            to,
            subject: '🎉 Welcome to Heguru Partnership Program (HPP)!',
            html: getEmailTemplate({
                title: 'Welcome to Ambassador Program',
                greeting: `Welcome, ${name}!`,
                body: `Congratulations on joining the Heguru Partnership Program (HPP)!
...
Share this code with friends and family to earn exclusive benefits. Every successful admission through your referral earns you rewards!`,
                ...
                ctaUrl: 'https://ambassador.heguru.in/refer',
...
${newStatus === 'Confirmed' ? 'Congratulations! This referral has been confirmed. Your benefits will be credited accordingly.' : 'We will keep you updated on further progress.'}`,
                ...
                ctaUrl: 'https://ambassador.heguru.in/referrals'
            })
        })
        return { success: true }
    } catch (error) {
    ...
    try {
        await resend.emails.send({
            ...
            html: getEmailTemplate({
                ...
                body: `Your one-time password for Heguru Ambassador Portal is:
